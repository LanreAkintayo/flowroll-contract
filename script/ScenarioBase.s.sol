...
abstract contract ScenarioBase is Script {
    ...
    uint256 constant CYCLE_DURATION = 2 minutes;
    ...
    function _setupEnvironment() internal {
        ...
        volatile    = MockPool(vm.envAddress("VOLATILE_POOL_ADDRESS"));
        payVault    = PayVault(vm.envAddress("PAY_VAULT_ADDRESS"));
    ...
    function _fundEmployer(address empAddress, uint256 amount) internal {
        vm.startBroadcast(deployerKey);
        usdc.mint(empAddress, amount);
        
        vm.stopBroadcast();
        console2.log("Employer", empAddress, "funded");
    }
    ...
    function _setupEmployer(EmployerSetup memory s) internal {
        vm.startBroadcast(s.employerKey);
