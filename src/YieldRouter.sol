...
contract YieldRouter is Ownable, Pausable, ReentrancyGuard {
    ...
    function startCycle(
        ...
        emit AgentAction(employer, newCycleId, 0, ActionType.CycleStarted, NO_POOL, NO_POOL, 0, 0, 0);
    }
     ...
     */
    function scorePool(uint256 poolIndex, uint256 idleAmount, uint256 timeLeft)
        public view returns (uint256 score)
    {
        ...
        uint256 riskMult  = _getRiskMultiplier(timeLeft);

        score = (((apyBps * liqFactor / SCALE) * riskMult / SCALE) * ilFactor) / SCALE;
    }

    function findBestPool(uint256 idleAmount, uint256 timeLeft)
        public view returns (uint256 bestIdx, uint256 bestScore)
    {
        bestIdx   = NO_POOL;
        bestScore = 0;
        for (uint256 i = 0; i < pools.length; i++) {
            uint256 s = scorePool(i, idleAmount, timeLeft);
            if (s > bestScore) { bestScore = s; bestIdx = i; }
        }
    }

    function findWorstAllocatedPool(address caller, uint256 cycleId, uint256 idleAmount, uint256 timeLeft)
        ...
        cycleIsActive(caller, cycleId)
    {
        ...
        if (timeLeft == 0) {
            ...
            emit AgentAction(caller, cycleId, timeIntoCycle, ActionType.PaydayTriggered, NO_POOL, NO_POOL, disbursed, 0, 0);
            return;
        }
        ...
        // ── Buffer adjustment ─────────────────────────────────────────────────
        {
            ...
            if (deployed > idleAmount) {
                ...
                uint256 withdrawn = _cascadeWithdraw(caller, cycleId, needed, idleAmount, timeLeft);
                emit AgentAction(caller, cycleId, timeIntoCycle, ActionType.BufferAdjusted, NO_POOL, NO_POOL, withdrawn, 0, 0);
            }
        }

        // ── No idle capital ───────────────────────────────────────────────────
        if (idleAmount == 0) {
            emit AgentAction(caller, cycleId, timeIntoCycle, ActionType.MovedToReserve, NO_POOL, NO_POOL, 0, 0, 0);
            return;
        }
        ...
        uint256 bestIdx;
        uint256 bestScore;
        (bestIdx, bestScore) = findBestPool(idleAmount, timeLeft);

        if (bestIdx == NO_POOL || bestScore == 0) {
            emit AgentAction(caller, cycleId, timeIntoCycle, ActionType.PoolBelowMinAPY, NO_POOL, NO_POOL, idleAmount, 0, 0);
            return;
        }

        // ── Rebalance or hold ─────────────────────────────────────────────────
        _handleRebalanceOrHold(caller, cycleId, timeIntoCycle, idleAmount, timeLeft, bestIdx, bestScore);
    }
    ...
    function _handleRebalanceOrHold(
        ...
        uint256 currentScore = _getCurrentAllocationScore(caller, cycleId, idleAmount, timeLeft);
        ...
        if (deployed == 0) {
            ...
            uint256 received;
            {
                (worstIdx,) = findWorstAllocatedPool(caller, cycleId, idleAmount, timeLeft);
    ...
    function _deployToPool(address caller, uint256 cycleId, uint256 poolIndex, uint256 amount) internal {
    ...
    function _cascadeWithdraw(
        ...
        while (remaining > 0) {
            (uint256 worstIdx,) = findWorstAllocatedPool(caller, cycleId, idleAmount, timeLeft);
    ...
    /**
     * @dev Risk thresholds are anchored to a 30-day reference cycle for scoring
     *      consistency across different cycle durations.
     *      High: >= 20 days, Med: >= 10 days, Low: < 10 days.
     */
    function _getRiskMultiplier(uint256 timeLeft) internal pure returns (uint256) {
        if (timeLeft >= 20 days) return RISK_MULT_HIGH;
        if (timeLeft >= 10 days) return RISK_MULT_MED;
...
//     // 25 days remaining, 10% buffer: allows aggressive yield farming when payday is far away
//     // 15 days remaining, 40% buffer: more conservative as payday approaches
//     // 5 days remaining, 105% buffer: full buffer, no yield farming — safety mode
...
//     function _isEarlyWithdrawalTrigger(
//         uint256 daysLeft
